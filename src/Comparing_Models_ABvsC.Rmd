...
```{r}
auc(ifelse(Aug_med_ABvsC.sum.7.2.test$Result_2016 == "AB", 0, 1), (as.numeric(Aug_med.ABvsC.pred.rf)-1))
```

```{r}
coords(Aug_med.ABvsC.mroc.rf, .9, "threshold", ret=c("sensitivity","specificity","ppv","npv"))
```
...
```{r}
auc(ordered(Aug_med_ABvsC.sum.7.2.test$Result_2016),
                               ordered(Aug_med.ABvsC.pred.ada$class))
```

```{r}
coords(Aug_med.ABvsC.mroc.ada, .9, "threshold", ret=c("sensitivity","specificity","ppv","npv"))
```
...
```{r}
auc(Aug_med_ABvsC.test_y.xgb,
    round(Aug_med.ABvsC.pred.xgb))
```

```{r}
coords(Aug_med.ABvsC.mroc.xgb, .9, "threshold", ret=c("sensitivity","specificity","ppv","npv"))
```
